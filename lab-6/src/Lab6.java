...
public class Lab6 extends GLJPanel implements GLEventListener {
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        double angle = 2 * Math.PI / numOfVertices;
        int radius = 6;
        float x = 0;
        float z = 0;

        for (var i = 0; i < 8; i += 4)
            gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT, materials[12], i);
        ...
        for (int i = 1; i <= numOfVertices; i++) {
            ...
            x = (float) (radius * Math.cos(angle * (i - 1)));
            z = (float) (radius * Math.sin(angle * (i - 1)));

            float x0 = (float) (radius * Math.cos(angle * i));
            float z0 = (float) (radius * Math.sin(angle * i));
